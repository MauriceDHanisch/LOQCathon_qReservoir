...
class PhotonicReservoirSimulator:
    ...
    def set_circuit_with_stored_layers(self, num_layers=None):
        ...
        if num_layers > len(self.layers):
            ...
            print(
                f"WARNING: Requested more layers than generated. Generating {missing_layers} more layers...")
    ...
    def full_layer_loss(self):
        ...
        """
        layer = pcvl.Processor("SLOS", self.m, source=pcvl.Source(
        ...
        if self.overlapping:
            ...
            if self.m % 2 != 0:
                print(
                    "WARNING: The number is not even and _overlapping is False => the last mode will be ignored.")
            for i in range((self.m)//2):
                layer = layer.add(2*i, self.U_ij_t(i, i+1, self.t))
        if len(self.noise_modes) != 0:
            random_noise = generate_gaussian_0_1(
                self.noise_mean, self.noise_std, len(self.noise_modes))
            for i in range(len(self.noise_modes)):
                layer.add(int(self.noise_modes[i]), pcvl.LC(
                    random_noise[i]), keep_port=True)
        return layer
    ...
    def sequential_expectation_with_memory(self, data, input_state, memory_length):
        ...
        for t in tqdm(range(self.t_max), desc="Processing time steps"):
            ...
            self.set_circuit_with_memory(
                memory_length=memory_length, num_layers=t)
        ...
        for t in tqdm(range(self.t_max), desc="Processing time steps"):
            ...
            if decay_rate is not None:
                ...
                for i in range(len(params_subset)):
                    decay_factor = (
                        1 / (decay_rate * (len(params_subset) - i) + 1))
